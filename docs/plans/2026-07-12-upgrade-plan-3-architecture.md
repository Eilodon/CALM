...
### (a) `repo_overview` compact mode  ⏱ ~2h — [XONG]
Param mới `compact: bool` default `false`: bỏ `workflow_guide` (client có AGENTS.md rồi), bỏ `entry_points`, `core_symbols` cap 8, `module_map` cap 10. `RepoOverviewParams` mới (orient.rs), cả 2 field bị bỏ dùng `#[serde(skip_serializing_if)]` nên key biến mất hẳn khỏi JSON thay vì `[]`/`null`. Test mới `repo_overview_compact_mode_drops_guide_and_caps_lists` xác nhận cả 2 cap + cả 2 key biến mất. AGENTS.md Stage 1 đã thêm ghi chú "Repeat calls in the same repo: dùng repo_overview(compact=true)". Gotcha thực thi: `edit_symbol` trên `repo_overview` bản thân nó bị CALM's own `validate_syntax` PARSE_ERROR giả (đã verify bằng cách compile y hệt new_text qua `rustc` độc lập — parse hoàn toàn hợp lệ) — không phải bug lần đầu, cùng loại với `calm-validate-syntax-false-positive-main-rs` đã biết, giờ xác nhận KHÔNG chỉ riêng main.rs. Workaround: dùng native `Edit` cho riêng file này, đã verify qua `edit_context` trước khi sửa. Đo thực tế qua daemon sống (kết nối trực tiếp unix socket, JSON-RPC raw, bypass MCP client-side schema cache của chính session này — session này init trước khi daemon respawn nên tool schema cached cũ không biết field `compact` mới, gọi qua `mcp__calm__repo_overview` bị coerce `true`→string rồi lỗi kiểu; đây là giới hạn client-cache, không phải bug server, đã verify bằng cách gọi thẳng socket): non-compact = 6592 bytes, compact = 2676 bytes (**giảm 59%**) trên CALM tự thân (199 file, 2958 symbol). `entry_points`/`workflow_guide` biến mất hoàn toàn khỏi key set; `module_map` 10, `core_symbols` 8 — đúng cap. Không đo token trực tiếp (không có tokenizer sẵn) nhưng tỉ lệ giảm tương đương ước tính plan gốc (mục tiêu ≤1K từ ~2.4K, tức ~58%) — khớp.

### (b) `suggested_next.gate` — phân biệt gate thật vs gợi ý mềm  ⏱ ~2h — [XONG]
Field additive `gate: Option<bool>` trên `SuggestedNext` (`common.rs`, ngay trước `suggested`/`suggested_with_args`) + helper mới `suggested_gated()`. `gate: Some(true)` cho 3 call site thực tế (không phải 2 như dự kiến ban đầu — 2 địa chỉ dòng trong plan gốc đã lệch do refactor edit.rs/guardrails.rs các phase trước; đối chiếu code thật thay vì tin số dòng cũ):
- `guardrails.rs` — `edit_context`'s own `suggested_next` khi risk cao (hint hướng tới `diff_impact`)
- `tools/edit.rs` `edit_lines_impl` — hint post-edit hướng tới `diff_impact` (dùng chung bởi cả `edit_lines` và `edit_symbol`)
- `tools/recover.rs` `session_context` — hint `pending_diff_impact` khi có file chưa qua `diff_impact` — PHÁT HIỆN MỚI không có trong plan gốc: đây là cùng một invariant hook-enforced (cùng flag `pending_diff_impact`), chỉ surface qua tool khác (agent check-in muộn thay vì thấy ngay trong response edit) — để `gate` unset ở đây sẽ SAI (ngụ ý advisory trong khi thực ra agent vẫn sẽ bị chặn ở PreToolUse hook nếu bỏ qua). Mọi hint khác (hotspots→file_overview, diff_impact's risk→callers, session_context frontier→file_overview, v.v.) giữ `gate: None` — verified bằng cách build lỗi bắt hết mọi struct-literal `SuggestedNext {}` còn thiếu field, không sót site nào. Test mới: `session_context_reports_and_clears_pending_diff_impact` bổ sung `gate == true`; `session_context_suggests_repo_overview_when_frontier_empty` bổ sung assert KHÔNG có key `gate`.

### (c) `scan_text.decode_scan_exhausted` — đã nằm trong Plan 2 §2.5 (chỉ nhắc để không làm trùng).

### (d) HMAC integrity cho `project_memory` (chống cấy note ngoài luồng)  ⏱ ~1 ngày · ưu tiên thấp nhất — [XONG]
Key 32B random tại `.calm/memory.key` (0600 trên Unix, tạo lazy — best-effort, không hard-fail nếu I/O lỗi); cột mới `content_mac` (nullable, migration qua `migrate_add_column`); `remember` ghi `HMAC-SHA256(topic ‖ 0x00 ‖ content)` (separator byte để tránh ambiguity giữa các cách chia topic/content khác nhau cho cùng 1 chuỗi nối); `recall` verify → field `integrity: "ok" | "unverified"(note cũ, pre-feature, content_mac NULL) | "mismatch"`; `related_notes` (`edit_context`/`locate`'s ambient-injection surface) **drop hẳn** note `mismatch` thay vì chỉ gắn nhãn — khác với `recall` (surface tường minh để agent tự phán đoán), vì đây là kênh passive mà agent không chủ động gọi.

Xác nhận qua code thật (không tin giả định trong plan gốc): `hash_content` (pipeline.rs) là FNV-1a KHÔNG keyed — dùng cho cache-invalidation, không phải MAC, nên nhánh "nếu blake3 sẵn" của plan gốc không áp dụng. Cargo.lock trước đó KHÔNG có `sha2`/`hmac` (dep hoàn toàn mới); `rand`/`getrandom` có sẵn nhưng chỉ transitive (chưa từng là dependency trực tiếp của crate nào) — đã thêm `hmac`+`sha2`+`rand` vào `[workspace.dependencies]` + `calm-core/Cargo.toml`. `.calm/` đã nằm trong `.gitignore` từ trước (dòng 24) nên `memory.key` tự động không bao giờ bị commit, không cần thêm entry mới.

Vị trí implement: `calm-core/src/memory.rs` (`load_or_create_mac_key`, `compute_mac`, `verify_integrity` — cùng module đã chứa `capture_refs`/`check_staleness`, tái dùng, không tạo module crypto riêng cho 3 hàm nhỏ). `rand` 0.9 đổi API so với 0.8: `OsRng` giờ chỉ implement `TryRngCore` (fallible) — dùng `try_fill_bytes` + propagate lỗi qua `io::Result` thay vì panic.

Test: 7 test mới trong `calm-core` (key persistence, per-project uniqueness, 0600 perms, MAC determinism + key/content/topic sensitivity, topic/content boundary ambiguity, ok/mismatch/unverified) + 4 test server-level trong `tools.rs` (remember→recall roundtrip "ok", out-of-band tamper→"mismatch", pre-feature note không có MAC→"unverified", `related_notes` drop hẳn note mismatch). Toàn bộ 20 test `remember`/`recall`/`related_notes` cũ vẫn pass nguyên (backward-compatible: note cũ không có `content_mac` đọc là "unverified", không phải lỗi).

Live dogfood qua daemon thật (raw socket, cùng cách đo §3.5(a)): `remember` lần đầu tự tạo `.calm/memory.key` (32 byte, mode 0600, lazy — file không tồn tại trước đó); `recall` note fresh → `integrity: "ok"`; tamper trực tiếp SQLite (`UPDATE project_memory SET content=...` qua kết nối riêng, mô phỏng ghi ngoài luồng MCP) → `recall` lại → `integrity: "mismatch"`, content vẫn hiển thị (transparent, đúng thiết kế — khác `related_notes` drop hẳn).
...
| Phần reindex+graph trong edit | _ms | < 150ms | **ĐO THẬT (Phase B T6.3, daemon log `edit_reindex_completed{reindex_ms,graph_mode}`):** 88ms (delta tối thiểu — edit README.md, `incremental`) · 188ms (edit fan-out tới file lớn nhất `parser.rs` qua `tokenize_identifier`, `incremental`) · 337ms (ca fallback `common.rs` → `new` fan-out 76 file > 50, `full_fallback`). **< 150ms đạt cho edit thường; edit fan-out nặng ~188ms; incremental nhanh 1.8–4× so với full ~337ms.** Floor bị chặn bởi 5 metric pass global (build_resolution_context + coreness/hub/boundary), cố ý giữ global theo D3. |
| Formal edges sống sót sau 1 edit (đếm `edge_confidence='formal'` trước/sau) | chết 100% | sống 100% trừ changed files | **ĐẠT (Phase B, commit 7df3800..efbd9d2).** Test T5 `scip_flag_survives_edit_of_other_file` chứng minh deterministic: `formal_source='scip'`/`ruled_out_by_scip` của edge NGOÀI delta sống nguyên qua incremental. Live: edit `tokenize.rs` giữ ~6287+ formal ngay (full rebuild sẽ về 0); chỉ ~451 formal của delta files phải chờ scip nền vá lại thay vì cả 6738 như full rebuild → giảm mạnh churn scip. |
...
| Golden equivalence 5-vòng | n/a | 3 lần chạy liên tiếp xanh | **ĐẠT (Phase B T1+T6.1, commit fe873bf/7df3800).** Hạ tầng golden xây ở T1 (semantic-key fingerprint D7). `golden_equivalence_incremental_vs_fresh_across_mutation_rounds`: 3 seed × 6 vòng = 18 so sánh incremental-vs-full xanh (permanent, CI). `..._on_real_calm_repo` (`#[ignore]`): 1 vòng trên copy repo CALM thật (body-edit + rename fan-out + delete) xanh (~206s) — đóng điều kiện (c) real-DB. A-1 chunk-boundary (`names_delta`>500) cũng có test riêng. |
