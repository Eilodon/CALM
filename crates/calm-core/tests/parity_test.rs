use calm_core::analysis::codeowners::load_codeowners;
use calm_core::analysis::coverage::load_coverage;
...
fn build_synthetic_db(conn: &rusqlite::Connection) {
    conn.execute_batch(
        "CREATE TABLE file_index (path TEXT PRIMARY KEY, hash TEXT NOT NULL, language TEXT, symbol_count INTEGER NOT NULL DEFAULT 0, last_indexed REAL NOT NULL, mtime REAL);
         CREATE TABLE symbols (id INTEGER PRIMARY KEY AUTOINCREMENT, qualified_name TEXT NOT NULL, name TEXT NOT NULL, kind TEXT NOT NULL, language TEXT NOT NULL, path TEXT NOT NULL, line_start INTEGER NOT NULL, line_end INTEGER NOT NULL, signature TEXT NOT NULL DEFAULT '', docstring TEXT NOT NULL DEFAULT '', name_tokens TEXT NOT NULL DEFAULT '', caller_count INTEGER NOT NULL DEFAULT 0, is_hub INTEGER NOT NULL DEFAULT 0, coreness INTEGER, possible_coreness INTEGER, is_entry_point INTEGER NOT NULL DEFAULT 0, is_test INTEGER NOT NULL DEFAULT 0, file_hash TEXT NOT NULL DEFAULT '', indexed_at REAL NOT NULL DEFAULT 0);
         CREATE TABLE call_edges (id INTEGER PRIMARY KEY AUTOINCREMENT, from_symbol TEXT NOT NULL, to_symbol TEXT NOT NULL, call_site_line INTEGER, edge_confidence TEXT NOT NULL DEFAULT 'textual', from_path TEXT, to_path TEXT, ruled_out_by_scip INTEGER NOT NULL DEFAULT 0);
...
fn test_coreness_parity() {
    ...
    let rust_coreness = calm_core::graph::coreness::compute_coreness(&conn).unwrap();
...
fn test_path_parity() {
    ...
    let result =
        calm_core::graph::path::bidirectional_bfs_path(&conn, "A", "C", 10, 3, 5000).unwrap();
